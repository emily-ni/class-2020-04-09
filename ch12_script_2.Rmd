...
The residual is the difference between the expected values and the actual values.

The school with the largest negative residual is Oberlin College.

The school with the largest positive residual is MCPHS University.

The model is missing what people are majoring in and the specialized colleges.
...
model_2 <- lm(earnings ~ public + price + price:public, data = x)
...
```{r, cache=TRUE}
x %>%
  rep_sample_n(size = nrow(x), reps = 1000, replace = TRUE) %>%
  group_by(replicate) %>%
  nest()  %>% 
